...
管理关注领域、候选主播和达人研究任务，读取用户账号正常可见的蝉妈妈榜单或主播页面，生成结构化达人拆解，并将候选主播加入后续录制流程。
...
LiveStream Agent Studio 提供本地工作台和工作流编排，但不会附带第三方账号、会员、云服务额度或外部录制软件。
...
LiveStream Agent Studio 不包含快抖直播录制助手，也不会自动下载或安装该软件。

安装完成后，请进入 LiveStream Agent Studio 的：

`设置与连接 → 快抖直播录制助手`

粘贴快抖直播录制助手 EXE 的完整本机路径，例如：D:\直播录屏\live-record-monitor\快抖直播录制助手.exe
如果需要从主播发现流程继续加入直播录制，还需要用户自行合法获取并安装直播录制工具。

当前支持在“设置与连接”中填写可选的快抖直播录制助手 EXE 路径。该软件属于第三方依赖，不包含在本项目和 Release 中；未配置时不会影响其他 Agent 的使用。
